...
| `intl_etf` | `IntlETFSubAgent` | "MAFANG performance", "Hang Seng premium", "run ETF premium backtest on MON100", "PELT regime backtest", "confidence threshold 60" |
...
- **Routing rules:** RULE 1a — backtest / walk-forward / PELT / confidence-threshold keywords → `run_ou_regime_backtest` (ETF Premium Strategy); RULE 1b — OU / mean-reversion / half-life keywords → `plot_ou_premium_chart`
