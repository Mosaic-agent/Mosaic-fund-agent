...
if [[ $# -eq 0 ]]; then
    ...
    if curl -s -I http://localhost:11434/ >/dev/null 2>&1; then
        echo "Local Ollama detected running on host. Starting clickhouse + ui + files server..."
        docker compose up -d clickhouse ui files 2>/dev/null
    else
        echo "Starting services (first run pulls gemma4 ~5-8 GB — grab a coffee)..."
        docker compose up -d clickhouse ollama ui files 2>/dev/null
