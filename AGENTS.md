# AGENTS.md - Mosaic-agent project instructions
...
python src/main.py import --category stocks --source nse
...
Stock and ETF imports use `shoonya`, `nse`, or `yfinance`. The selected source
is saved in ClickHouse for 24 hours and reused until expiry; the CLI prompts
with choices 1-3 only when no valid saved preference exists.
...
- **Anomaly pipeline:** `run_composite_anomaly` (`src/ml/anomaly/`) is a **6-step** composite: (1) MAD robust Z, (2) GARCH(1,1) standardised residuals, (3) Isolation Forest confidence multiplier, (4) PELT change-point detection (`ruptures`, rbf cost), (5) Company Event classification, **(6) VolumeHMMStrategy — 2-component GMM on log(volume) → `p_institutional` column (float 0–1)**. `p_institutional > 0.70` flags institutional block deals even when price barely moves. Key params: `volume_z_threshold` (fixed z cutoff, default None), `volume_hmm_threshold` (GMM posterior cutoff, default 0.70). Both can be set in `.env` as `ANOMALY_VOLUME_Z_THRESHOLD` / `ANOMALY_VOLUME_HMM_THRESHOLD`. Automatically stores anomalies in Qdrant if `symbol` and `category` are passed. Gracefully falls back if `arch`/`ruptures`/`sklearn` missing or <60 rows.
- **Anomaly tools (two separate tools):**
  ...
  - `search_anomaly_events(symbol, days, volume_z_threshold, volume_hmm_threshold)` (`market/equity.py`) — equity-generic; loads corp actions from ClickHouse → runs pipeline with suppression → **parallel** Google News (ThreadPoolExecutor, ±1d fallback, NewsAPI for <30d dates). Output includes `p_institutional` on every flagged row. Call for any NSE/BSE stock anomaly investigation.
...
- **AMC holdings fetcher:** `src/importer/fetchers/amc_holdings_fetcher.py` — canonical entry point for all 4 AMC importers (nippon, dsp, icici, icici-index). Use `fetch_amc_holdings(amc, full_reimport=, dry_run=, max_workers=)` or `python src/main.py import --category nippon|dsp|icici|icici-index`. The `scripts/fund_imports/` factory is the implementation layer; `amc_holdings_fetcher` is the public API. AMC monthly sources parse with 2 bounded workers by default; use `AMC_IMPORT_MAX_WORKERS` or the canonical `max_workers` argument (1–4) to tune an individual run.
...
Apply these rules in every Codex session for this project:
...
and passed in as pre-computed results. Codex is only allowed
...
Use the repository skills under `.agents/skills/` when their trigger matches the
request:

| Skill | Trigger / Purpose |
|---|---|
| `/commit` | Stage and commit changes using Conventional Commits. |
| `/daily-signal-composite` | Compute the unified ETF signal dashboard. |
...
The project also uses these persistent guidance files:
