...
_SYNTHESIS_PROMPT = (
    ...
    "structured Markdown research note with these sections:\n\n"
    "(1) Company Snapshot — key metrics table (Current Price, Market Cap, SEBI Cap Category, AMFI Statutory Rank & 6M Avg Mcap, 52W High/Low, Sector/Industry); write `[CHART:price]` on its own line immediately after the table where the price & anomaly chart should appear\n"
    "(1b) Price Anomalies & Shock Events — summarise the detected anomaly dates/red dots on the chart, regime, price return %, volume spikes, and underlying news/macro causes from the anomalies dataset\n"
    "(1c) Event Correlation Analysis — include key event attributions and timeline from the correlations dataset; write `[CHART:correlation_timeline]` and `[CHART:lead_lag_grid]` on their own lines\n"
    ...
    "(5) Institutional Ownership — write `[CHART:shareholding]` on its own line, then promoter/FII/DII/Public % "
