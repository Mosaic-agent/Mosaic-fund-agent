...
class SignalSubAgent(_SubAgent):
    ...
    SYSTEM_PROMPT = (
        ...
        "the system re-fetches if the DB copy is older than 2 minutes (Kite → AMC APIs → NSE). "
        "Tool output includes an `inav_source` field: 'db' = cached, 'kite_live' / "
        "'nippon_amc_live' / 'zerodha_amc_live' / 'mirae_amc_live' = live AMC iNAV, "
        "'nse_prev_nav' = prior-day declared NAV (correct for US/HK-market ETFs). "
        "Always report which source was used and the snapshot timestamp.\n\n"
        "## Charts\n"
