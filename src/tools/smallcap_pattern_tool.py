...
───────────────────────────────────
LangChain @tool wrappers for the Multi-AMC Cap-Segment Pattern Analyzer
(Small / Mid / Large Cap), all backed by MFCapPatternAnalyzer.

Allows MosaicFundAgent and subagents to invoke cap-segment quantitative
analysis directly.
"""
...
@tool
def analyze_midcap_patterns(
    amc: str = "all",
    lookback_months: int = 24,
) -> str:
    """
    Run Multi-AMC Mid Cap pattern and institutional shift analyzer.

    Returns EOD ETF price trends, AMFI mutual fund category inflows,
    Mid Cap portfolio holdings across AMCs (Nippon, DSP, HDFC, Quant, ICICI, Kotak, Bajaj, or ALL),
    MoM net additions & trims, persistence-ranked multi-AMC cross-conviction
    (names held by multiple AMCs continuously vs. a recent single-month pile-in),
    and composite quant signals.
    ...
    """
    try:
        analyzer = MFCapPatternAnalyzer(category="mid")
        report = analyzer.analyze(amc=amc, lookback_months=lookback_months)
        return analyzer.render_ascii_dashboard(report)
    except Exception as exc:
        logger.error("Error running analyze_midcap_patterns: %s", exc)
        return f"Error analyzing Mid Cap patterns for AMC '{amc}': {exc}"


@tool
def analyze_largecap_patterns(
    amc: str = "all",
    lookback_months: int = 24,
) -> str:
    """
    Run Multi-AMC Large Cap pattern and institutional shift analyzer.

    Returns EOD ETF price trends, AMFI mutual fund category inflows,
    Large Cap portfolio holdings across AMCs (Nippon, DSP, HDFC, Quant, ICICI, Kotak, Bajaj, or ALL),
    MoM net additions & trims, persistence-ranked multi-AMC cross-conviction
    (names held by multiple AMCs continuously vs. a recent single-month pile-in),
    and composite quant signals.
    ...
    """
    try:
        analyzer = MFCapPatternAnalyzer(category="large")
        report = analyzer.analyze(amc=amc, lookback_months=lookback_months)
        return analyzer.render_ascii_dashboard(report)
    except Exception as exc:
        logger.error("Error running analyze_largecap_patterns: %s", exc)
        return f"Error analyzing Large Cap patterns for AMC '{amc}': {exc}"
